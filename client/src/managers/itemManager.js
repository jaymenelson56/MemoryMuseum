...
export const getPendingItems = async (id) => {
  return await fetch(_apiUrl + `/needing-approval/${id}`).then((res) =>
    res.json()
  );
};
...
export const changeExhibit = async (id, newExhibitId, loggedInUserId) => {
  const formData = new FormData();
  formData.append("newExhibitId", newExhibitId);
  formData.append("loggedInUserId", loggedInUserId);

  const response = await fetch(`${_apiUrl}/change-exhibit/${id}`, {
    method: "PUT",
    body: formData,
  });

  if (!response.ok) {
    throw new Error("Failed to change exhibit");
  }
